...
if st.button("⚡ EXECUTE REAL-TIME RADAR CONFLUENCE SCAN", use_container_width=True):
    ...
    # Clean rendering for standard info
    st.markdown(f'<div style="background-color: #0b132b; padding: 10px; border-radius: 8px; border-left: 4px solid #00f0ff; margin-bottom: 20px; font-size:12px; color:#94a3b8;"><b>📡 DATA SOURCE:</b> {source_label} | <b>⏱️ CLOCK:</b> {current_hour:02d}:{current_min:02d}:{current_sec:02d} (UTC+5)</div>', unsafe_allow_html=True)
    ...
    for strategy_name, result in strategy_checklist.items():
        ...
        st.markdown(f'<div style="display: flex; justify-content: space-between; background-color: {bg}; padding: 8px 15px; margin-bottom: 5px; border-radius: 6px; border: 1px solid {color}; font-size: 14px; font-weight: bold;"><span style="color:#cbd5e1;">{strategy_name}</span><span style="color:{color};">{result}</span></div>', unsafe_allow_html=True)
    ...
    # SYSTEM BOUNDARY RE-STYLED TO SOLVE NESTING SYNTAX BREAKAGE Completely
    st.markdown(f'<div style="background-color:{bg_rgba}; padding: 25px; border-radius: 14px; border: 2px solid {color_hex}; text-align: center; margin-bottom:20px;"><h2 style="color:{color_hex}; margin:0; font-size:38px; font-weight:900; letter-spacing:1px;">🎯 STRATEGY DIRECTIVE: {final_decision}</h2><hr style="border-color:{color_hex}; opacity:0.3; margin:15px 0;"><p style="margin: 0; font-size:16px; color:#ffffff; font-weight:bold;">{time_instruction}</p><p style="margin: 4px 0 15px 0; font-size:14px; color:#94a3b8;">{setup_instruction}</p><p style="margin: 0; font-size:15px; color:{color_hex}; font-weight: 900; background:rgba(0,0,0,0.4); padding:12px; border-radius:8px; border: 1px dashed {color_hex};">{action_window}</p></div>', unsafe_allow_html=True)

    st.markdown("<p style='text-transform:uppercase; font-weight:900; font-size:12px !important; letter-spacing:1px; color:#64748b;'>💡 STRATEGIC JUSTIFICATION MATRIX</p>", unsafe_allow_html=True)
    for reason in dynamic_reasons:
        st.markdown(f'<div style="background-color: #0b1220; border-left: 4px solid {color_hex}; padding: 12px; margin-bottom: 8px; border-radius: 6px; font-size: 14px;">{reason}</div>', unsafe_allow_html=True)
