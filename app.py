...
st.caption("Advanced Confluence Scanning Engine • Forex & Gold")
st.markdown("---")

# 1. Inputs Selector Block
asset_selection = st.selectbox("💱 Select Instrument Pair", ["EURUSDT", "GBPUSDT", "AUDUSDT", "EURGBP", "XAUUSDT"])
...
if st.button("🔍 Execute Algorithmic Deep Scan", use_container_width=True):
    with st.spinner("Streaming orderbook data..."):
        try:
            ...
            endpoint_url = f"https://fapi.binance.com/fapi/v1/klines?symbol={asset_selection}&interval={timeframe_selection}&limit=100"
            raw_feed = requests.get(endpoint_url).json()
            
            # Parsing OHLC candle array structure
            ...
            # Condition B: Overbought/Oversold verification
            if current_rsi < 35: conv_score += 1
            elif current_rsi > 65: conv_score -= 1
                
            # Condition C: Proximity zone to Key Price Levels
            if (live_price - structural_support) < (structural_resistance - live_price):
                conv_score += 1  # Price is situated close to structural floor
            else:
                conv_score -= 1  # Price is pressing close to ceiling resistance
            ...
            if conv_score >= 2:
                ...
                st.info(f"💡 **Analysis:** Market is highly oversold ({current_rsi:.1f}) and sitting close to major historical support ({structural_support:,}). Structure indicates an upward bounce.")
            elif conv_score <= -2:
                st.error("🔴 **STRONG DOWN (SELL CONFLUENCE)**")
                st.info(f"💡 **Analysis:** Price is overextended ({current_rsi:.1f}) and directly approaching key structural resistance ({structural_resistance:,}). Downward rejection is likely.")
            else:
                st.warning("⚪ **NEUTRAL (CONSOLIDATION NO-TRADE ZONE)**")
                st.info("💡 **Analysis:** Moving averages and structural levels are conflicting. The market is tracking completely sideways inside a tight range.")
