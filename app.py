...
if st.button("⚡ EXECUTE REAL-TIME RADAR CONFLUENCE SCAN", use_container_width=True):
    ...
    st.markdown(f"""
    <div style="background-color:{bg_rgba}; padding: 25px; border-radius: 14px; border: 2px solid {color_hex}; text-align: center;">
        ...
        <p style="margin: 0; font-size:15px; color:{color_hex}; font-weight: 900; background:rgba(0,0,0,0.4); padding:12px; border-radius:8px; border: 1px dashed {color_hex};">
            {action_window}
        </p>
    </div>
    """, unsafe_allow_html=True)
