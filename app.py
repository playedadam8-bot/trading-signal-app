...
if st.button("⚡ GET SIGNAL", use_container_width=True):
    ...
    st.markdown(f"""
        <div style="background: linear-gradient(90deg, #1e1b4b, #2e0854); padding: 12px; border-radius: 8px; border: 1px dashed #00f0ff; text-align: center; margin-bottom: 15px;">
            <span style="color: #94a3b8; font-size: 13px; font-weight: bold;">⏱ ... LOCAL TIME (UTC+5): {current_hour:02d}:{current_min:02d}:{current_sec:02d}</span>
            <span style="color: #00ffcc; font-size: 14px; font-weight: 900; margin-left: 15px;">📡 SOURCE: {source_label}</span>
        </div>
    """, unsafe_allow_html=True)

    st.markdown("### 🚦 SHAWKAT TRADEZ DECISION RADAR")
    ...
    <div style="background-color:{bg_rgba}; padding: 25px; border-radius: 12px; border: 2px solid {color_hex}; text-align: center;">
        <h2 style="color:{color_hex}; margin:0; font-size: 32px; font-weight:900;">🎯 DIRECTION: {final_decision} ({'UP' if final_decision=='CALL' else 'DOWN'})</h2>
